...
四问求解代码位于 `src/solvers`；九幅图的独立作图代码位于 `src/figures`。公共几何和协议组件位于 `src/common`，离线模拟与结果汇总位于 `src/experiments`。算法依据 `docs/B题解题思路.md`、题面及附件；官方测试由用户在模拟器界面启动，本项目默认运行离线验证。
...
```powershell
.venv/Scripts/python -m solvers.problem1 --input examples/problem1.json
.venv/Scripts/python -m solvers.problem2 --input examples/problem2.json
.venv/Scripts/python -m solvers.problem3 --seed 20260910
.venv/Scripts/python -m solvers.problem4 --seed 20260910
```
...
from solvers.problem3 import solve

result = solve(Client(random_scene(20260910, 3)), Config())
```
...
```powershell
.venv/Scripts/python -m solvers.problem3 --backend http --robot-id "实际参赛队号" --source official_practice --output outputs/official/p3_run01.json
.venv/Scripts/python -m solvers.problem4 --backend http --robot-id "实际参赛队号" --source official_practice --output outputs/official/p4_run01.json
```

地址默认 `http://127.0.0.1:2026`，可用 `--base-url` 修改。`--source` 只标记日志来源，不控制模拟器模式。正式模式需在界面选择，代码不会启动正式测试，也不会下载、解析或修改官方加密日志。
...
.venv/Scripts/python -m experiments.summarize
```
...
```powershell
.venv/Scripts/python -m figures.fig01_localization
.venv/Scripts/python -m figures.fig02_circle_counterexample
.venv/Scripts/python -m figures.fig03_candidate_region
.venv/Scripts/python -m figures.fig04_omni_coverage
.venv/Scripts/python -m figures.fig05_directional_coverage
.venv/Scripts/python -m figures.fig06_omni_trajectory
.venv/Scripts/python -m figures.fig07_mixed_trajectory
.venv/Scripts/python -m figures.fig08_time_breakdown
.venv/Scripts/python -m figures.fig09_trial_distribution
```
...
`outputs/` 为可重建输出，默认不纳入 Git；`src/`、`tests/`、`examples/`、依赖和说明文件应纳入支撑材料。原有题目资料和解题思路文档保持原样。
