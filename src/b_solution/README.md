...
默认运行本次优化策略；`--previous` 切换优化前的改进策略，`--baseline` 切换基础策略，两者不能同时使用。默认输出在项目目录的 `outputs/runs` 下；可用 `--output` 指定绝对路径。路径含空格时请用引号包住。Windows 的 Python 代码路径推荐 `Path(r"D:\建模\B题")` 或 `Path("D:/建模/B题")`。不要把压缩包内部路径当作已经解压的文件路径。
...
每问使用 12 个常规场景及 4 个联合压力场景，默认比较 `improved`（旧选点策略）和 `optimized`（新策略），共 64 次运行，两组均使用当前几何实现。`--models baseline improved` 可比较原来的两种策略。固定报告的历史结果仍保存在 `results`，没有用新数据覆盖；几何实现数值变化可能改变后续观测位置，因此新运行不保证逐动作复现历史轨迹。JSON 中的 `case_sha256` 是自建场景参数摘要，绝不是官方案例编码。

独立种子及扩展误差场景：

```bash
python -m b_solution benchmark --cases 6 --stress-cases 4 --seed-start 200 --extended --output outputs/optimization/holdout_final
```

`--extended` 增加最小接收半径下的空间突变误差和零误差场景。单局失败会保留失败原因并继续运行；汇总提供整局成功率、最小清除比例、成功局耗时的均值/中位数/P95/最大值。比较耗时必须同时查看成功率，不能把未完成局当成快速完成局。
...
| core/planning.py | 接收状态情景、保守光学分块、开放路线优化 |
...
新策略记录正读数、负读数和失败清除。第三问的负读数提供 1000 米排除圆；第四问的负读数仅用于有限接收半径/发射轴情景评分，不据此删除认证位置。失败清除对两问均提供 20 米排除圆。只有整个光学分块落入排除圆时才跳过该块。

选点前瞻包含正负 1 度和零误差情景、无信号后续代价，以及失去信号后的接收点附近候选。概率仅表示有限假设的比例，不是官方分布或连续最坏情况证明。光学兜底对当前完整可行多边形递归分块，每块顶点到清除点的距离均不超过 19.5 米。路线保持全部剩余分块；10 次局部测量上限防止反复试探。覆盖站数和终止条件不变。

最小包围圆使用确定性打乱的增量算法，保留穷举参考用于退化回退和测试；最终始终复算全部顶点距离。缓存仅在正读数改变多边形后失效。批量裁剪跳过原多边形已满足的半平面。同点共享测量最多增加两个有收益的待定位频道；所有接口仍串行发送。

达到现实截止时间或收到矛盾反馈时程序输出失败记录，不能声称已清除所有目标。HTTP 重试复用同一请求，并受剩余现实时间限制；发出动作前检查包含移动在内的虚拟时间上界。本地模拟器本身没有完整模拟官方超时和网络，通信故障由测试中的模拟传输注入。
