...
DOCS_ROOT = PROJECT_ROOT / 'docs' / 'b_solution'
TMP = PROJECT_ROOT / 'outputs' / '.cache' / 'b_report_assets'
OUT = PROJECT_ROOT / 'outputs' / 'report'
WIDTH = 6.94
...
def compile_source():
    summary = json.loads((PACKAGE_ROOT / 'results' / 'summary.json').read_text())
    ...
    audits = json.loads((PACKAGE_ROOT / 'results' / 'data_audit.json').read_text())
    ...
    candidates = json.loads((PACKAGE_ROOT / 'results' / 'q2_candidates.json').read_text())
    ...
    source = (DOCS_ROOT / 'report_template.md').read_text(encoding='utf-8')
    for key, value in replacements.items():
        source = source.replace('{{' + key + '}}', value)
    if re.search(r'\{\{[^}]+\}\}', source):
        raise ValueError('Unresolved report template field')
    (DOCS_ROOT / 'report.md').write_text(source, encoding='utf-8')
    return source
...
<alias><family>黑体</family><prefer><family>Droid Sans Fallback</family></prefer></alias>
    ...
    while i < len(lines):
        ...
        if line.startswith('# '):
            ...
            imgpath = PACKAGE_ROOT/match.group(2)
