...
from experiments.shared_environment import Scenario, SharedSimulator, make_scenario


def _load_b_solution():
    b_root = Path(__file__).resolve().parents[2] / "b_solution"
    if str(b_root) not in sys.path:
        sys.path.insert(0, str(b_root))
    return importlib.import_module("protocol").Session, importlib.import_module("solver").Solver
...
def _run_main(scenario: Scenario, variant: str) -> dict:
    row = _base_row(scenario, "main_solution", variant)
...
def _paired(rows: list[dict]) -> list[dict]:
    ...
    for problem, profile, seed, variant in keys:
        ...
        if set(current) != {"main_solution", "b_solution"}:
            continue
        main, b = current["main_solution"], current["b_solution"]
        both_success = main["success"] and b["success"]
        if both_success:
            delta = b["virtual_time_s"] - main["virtual_time_s"]
            winner = "b_solution" if delta < -1e-9 else "main_solution" if delta > 1e-9 else "tie"
        elif main["success"] and not b["success"]:
            delta, winner = None, "main_solution"
        elif b["success"] and not main["success"]:
        ...
        pairs.append(
            {
                ...
                "seed": seed,
                "variant": variant,
                "main_success": main["success"],
                "b_success": b["success"],
                "both_success": both_success,
                "main_time_s": main["virtual_time_s"],
                "b_time_s": b["virtual_time_s"],
                "b_minus_main_time_s": delta,
                "winner": winner,
...
def _analysis(summary: list[dict], pairs: list[dict], scenarios: list[dict]) -> str:
    ...
    lines.extend(["", "## 配对结论", "", "|问题|场景|变体|两者均成功|主方案时间/s|b_solution时间/s|b−主方案/s|配对胜者|", "|---:|---|---|---:|---:|---:|---:|---|"])
    for row in pairs:
        lines.append(
            f"|{row['problem']}|{row['profile']}|{row['variant']}|{'是' if row['both_success'] else '否'}|"
            f"{row['main_time_s'] if row['main_time_s'] is not None else '—'}|"
            f"{row['b_time_s'] if row['b_time_s'] is not None else '—'}|"
            f"{row['b_minus_main_time_s'] if row['b_minus_main_time_s'] is not None else '—'}|{row['winner']}|"
        )
    complete_pairs = [row for row in pairs if row["both_success"] and row["b_minus_main_time_s"] is not None]
    lines.extend(["", "## 自动分析", ""])
    if complete_pairs:
        b_wins = sum(row["winner"] == "b_solution" for row in complete_pairs)
        main_wins = sum(row["winner"] == "main_solution" for row in complete_pairs)
        ties = len(complete_pairs) - b_wins - main_wins
        mean_delta = float(np.mean([row["b_minus_main_time_s"] for row in complete_pairs]))
        lines.append(f"在 {len(complete_pairs)} 个双方均成功的配对中，b_solution 更快 {b_wins} 次，主方案更快 {main_wins} 次，持平 {ties} 次；平均时间差 b−主方案为 {mean_delta:.3f} s。")
    else:
    ...
    for problem in (3, 4):
        for profile in ("random", "boundary"):
            ...
            if not group:
                continue
            main_ok = sum(r["main_success"] for r in group)
            b_ok = sum(r["b_success"] for r in group)
            lines.append(f"问题{problem} / {profile}：主方案成功 {main_ok}/{len(group)}，b_solution 成功 {b_ok}/{len(group)}。")
    lines.extend([
        "",
        "## 解读边界",
        "",
...
def run(output: str = "outputs/shared_comparison", trials: int = 10, start_seed: int = 20260910, include_baseline: bool = True) -> dict:
    ...
    for scenario in scenarios:
        for variant in variants:
            main_row = _run_main(scenario, variant)
            b_row = _run_b_solution(scenario, variant)
            rows.extend((main_row, b_row))
            print(
                f"P{scenario.problem} {scenario.profile} seed={scenario.seed} {variant}: "
                f"main={main_row['cleared_count']}/{main_row['true_count']} "
                f"b={b_row['cleared_count']}/{b_row['true_count']}",
                flush=True,
            )
    pairs = _paired(rows)
