...
import numpy as np
from geometry import minimum_circle, safe_second_point, unit
from solver import coverage_sites
from preprocess_logs import audit
...
def main():
    ...
    rows = json.loads((BASE / 'results' / 'raw_results.json').read_text())
    ...
    examples = json.loads((BASE / 'results' / 'examples.json').read_text())
    ...
    (BASE / 'results' / 'data_audit.json').write_text(json.dumps(audits, ensure_ascii=False, indent=2), encoding='utf-8')
